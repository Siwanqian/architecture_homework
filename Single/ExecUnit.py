...
class FPAdder:
    ...
    def execute(self, cdb: CDB, reservation_station): # 执行操作，将剩余的执行周期减1，如果执行完成则广播数据
        ...
        if self.busy and self.remain_time == 0 and not cdb.is_busy():
            value = '{}+{}'.format(self.Vj, self.Vk) if self.Op == 'fadd,d' else '{}-{}'.format(self.Vj, self.Vk)
...
class FPMultiplier:
    ...
    def execute(self, cdb: CDB, reservation_station): # 将指令发射入执行单元并对执行单元设忙碌
        ...
        if self.busy and self.remain_time == 0 and not cdb.is_busy():
            value = '{}*{}'.format(self.Vj, self.Vk) if self.Op == 'fmul.d' else '{}/{}'.format(self.Vj, self.Vk)
            cdb.broadcast(self.dest, value)
            self.busy = False
            return
...
class MemoryUnit:
    def __init__(self):
        self.Mem = {'Regs[x3]+44':  'Mem[Regs[x3]+44]', 'Regs[x2]+32':  'Mem[Regs[x2]+32]'}
        self.busy = False
        self.addr = None
        self.dest = None
